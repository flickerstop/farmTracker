...
function load() {
    ...
    if (typeof (Storage) !== "undefined") {
        ...
        if (loadFile != null) {
            ...
            save("No save found, created default");
        }
    } else {
        window.alert("Web Storage is not supported!");
    }
...
function checkIfMobile() {
    ...
    // If mobile, set the body to be mobile friendly
    if(check){
        d3.select("#body").attr("id", "body-mobile");
    }
}
...
function getVersionNumber() {
    $.get("/get/version", function(data) {
        versionNumber = `v${data.major}.${data.minor}.${data.patch}`;
        d3.select("#topBar-left").html(versionNumber);
    });
}
